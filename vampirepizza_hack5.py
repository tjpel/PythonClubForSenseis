...
lvl1_enemy_types.append(VampireSprite)
lvl1_enemy_types.append(WerePizza)
...
def run_level(enemy_list, start_bucks, clear_tiles): 
    ...
    counters = Counters(start_bucks, BUCK_RATE, BUCK_BOOSTER, WIN_TIME)
    ...
    #Set up collision detection
        ...
        for vampire in all_vampires:
            ...
            if -1 < vampire_right_side_x < 10:
                right_tile = tile_row[vampire_right_side_x]
            else:
                right_tile = None
            if bool(left_tile):
                vampire.attack(left_tile)
            if bool(right_tile) and right_tile.rect.x != left_tile.rect.x:
    ...
    #print(running, exited)
    return running, exited, counters
...
level_config = [
    [lvl1_enemy_types, LVL1_STARTING_BUCKS],
    [lvl2_enemy_types, LVL2_STARTING_BUCKS]
]
end_font = pygame.font.Font('pizza-font.ttf', 50)
exited = True
level_cursor = 0
while exited and level_cursor < len(level_config):
    if level_cursor > 0:
        clear_tiles = True
    else:
        clear_tiles = False
    running, exited, counters = run_level(level_config[level_cursor][0], level_config[level_cursor][1], clear_tiles)
    if exited:
        if counters.bad_reviews >= MAX_BAD_REVIEWS:
            end_surf = end_font.render('Game Over', True, WHITE)
            GAME_WINDOW.blit(end_surf, (350, 200))
            display.update()
        elif level_cursor < len(level_config):
            cont_surf = end_font.render('Press Enter/Return for Level ' + str(level_cursor + 1), True, WHITE)
            ...
            end_surf = end_font.render('You Won!!!')
            GAME_WINDOW.blit(end_surf, (350, 200))
            display.update()
        level_cursor += 1
