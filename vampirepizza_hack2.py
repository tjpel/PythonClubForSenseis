...
# TABLE.set_alpha(127)
cannon_img = image.load('anchovy-cannon.jpg')
cannon_surf = Surface.convert(cannon_img)
CANNON = transform.scale(cannon_surf, (WIDTH, HEIGHT))
CANNON.set_alpha(127)
anchovy_img = image.load('anchovy.png')
anchovy_surf = Surface.convert(anchovy_img)
ANCHOVY = transform.scale(anchovy_surf, (20, 20))
#ANCHOVY.set_alpha(127)

#Image of pizza being destroyed
# destroy_img = image.load('destroy.png')
# destroy_surf = Surface.convert(destroy_img)
# DESTROY = transform.scale(destroy_surf, (WIDTH, HEIGHT))
    ...
    for column in range(11):
        if column <= 1:
            ...
            if row == 5:
                if 1 < column < 6:
                    new_tile = ButtonTile()
                    new_tile.trap = [SLOW, DAMAGE, EARN, PROJECTILE][column - 2]
                else:
                    new_tile = InactiveTile()
            else:
                new_tile = PlayTile()
